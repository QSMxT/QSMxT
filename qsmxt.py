...
from interfaces import nipype_interface_analyse as analyse
...
from workflows.template import init_template_workflow
...
def init_session_workflow(args, subject, session):
    ...
    if any([args.do_qsm, args.do_t2starmap, args.do_r2starmap, args.do_swi]):
        ...
        wfs = init_workflow_and_add_nodes(args, subject, session, phase_run_details, init_qsm_workflow)
    ...
    if args.do_segmentation:
        ...
        wfs = init_workflow_and_add_nodes(args, subject, session, t1w_run_details, init_segmentation_workflow)
        wf.add_nodes([wfi for wfi in wfs if wfi])

    if args.do_qsm and args.do_segmentation and args.do_analysis:
        qsm_output_nodes = []
        seg_output_nodes = []
        for node in wf._get_all_nodes():
            if 'qsmxt_outputs' in node._name:
                qsm_output_nodes.append(node)
            if 'seg_outputs' in node._name:
                seg_output_nodes.append(node)
        ...
        n_merge_seg = Node(Merge(len(seg_output_nodes)), name="merge_seg")
        for i, node in enumerate(seg_output_nodes):
            wf.connect(node, 'qsm_segmentation', n_merge_seg, f'in{i+1}')
        
        n_analyse_qsm = MapNode(
            interface=analyse.AnalyseInterface(
                in_labels=args.labels_file
            ),
            iterfield=['in_file', 'in_segmentation'],
            name='analyse_qsm'
        )
        wf.connect([
            (n_merge_seg, n_analyse_qsm, [('out', 'in_segmentation')]),
            (n_merge_qsm, n_analyse_qsm, [('out', 'in_file')])
        ])
        ...
        wf.connect([
            (n_analyse_qsm, n_datasink, [('out_csv', 'analysis')])
        ])
...
def parse_args(args, return_run_command=False):
    ...
    parser.add_argument(
        '--labels_file',
        default=os.path.join(get_qsmxt_dir(), 'aseg_labels.csv'),
    ...
    if os.path.exists(os.path.join(args.output_dir, 'settings.json')):
        ...
        if using_json_settings:
            ...
            for key in keys:
                ...
                vars(args)[key] = json_settings[key]
                final_args[key] = json_settings[key]
...
def get_interactive_args(args, explicit_args, implicit_args, premades, using_json_settings):
    ...
    def update_desired_images():
        ...
        while True:
            ...
            if 'seg' in user_in.split():
                ...
                explicit_args['do_segmentation'] = False
            if 'analysis' in user_in.split():
                ...
                explicit_args['do_analysis'] = False
            if 'template' in user_in.split():
                ...
                explicit_args['do_template'] = False
    ...
    while True:
        ...
        print(f" - Segmentations: {'Yes' if args.do_segmentations else 'No'}")
