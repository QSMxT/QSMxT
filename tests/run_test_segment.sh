...
sed -i 's/16/4/g' /tmp/${timeStamp}/QSMxT/interfaces/nipype_interface_fastsurfer.py
...
[ -f  /tmp/${timeStamp}/03_segmentation/t1_segmentations/aparc.DKTatlas+aseg.deep_nii.nii ] && echo "$FILE exist." || exit 1
[ -f  /tmp/${timeStamp}/03_segmentation/qsm_segmentations/aparc.DKTatlas+aseg.deep_nii_trans.nii ] && echo "$FILE exist." || exit 1