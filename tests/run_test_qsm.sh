...
sudo docker run --env BRANCH="${BRANCH}" --env PYTHONPATH=/tmp/QSMxT --env DOWNLOAD_URL="${DOWNLOAD_URL}" --env DATA_PASS="${DATA_PASS}" --env UPLOAD_URL="${UPLOAD_URL}" -v /tmp:/tmp ${container} pytest /tmp/QSMxT/tests/run_test_qsm.py -s
