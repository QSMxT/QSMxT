...
sudo docker run --env PYTHONPATH=/tmp/QSMxT --env DATA_URL="${DATA_URL}" --env DATA_PASS="${DATA_PASS}" --env UPLOAD_URL="${UPLOAD_URL}" --env UPLOAD_PASS="${UPLOAD_PASS}" -v /tmp:/tmp ${container} pytest /tmp/QSMxT/tests/run_test_qsm.py -s
