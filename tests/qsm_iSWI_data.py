...
if __name__ == "__main__":
    ...
    for thresh in np.arange(0.0, 0.8, 0.1):
        current_dir = out_dir + f"/one_thresh_masking_{int(100*thresh)}"
        if not os.path.isdir(current_dir + "/qsm/qsm_final"):
    ...
    for th1 in np.arange(0.1, 0.8, 0.1):
        for th2 in np.arange(0.1, 0.8, 0.1):
            current_dir = out_dir + f"/two_thresh_masking_{int(100*th1)}_{int(100*th2)}"
