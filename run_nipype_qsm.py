...
bet = MapNode(BET(frac=0.4, mask=True, robust=True), name="bet", iterfield=['in_file'])
phs_range = MapNode(ImageMaths(op_string='-div 4096 -mul 6.28318530718 -sub 3.14159265359'), name="phs_range", iterfield=['in_file'])

# Connect all components of the preprocessing workflow
preproc.connect([(infosource, selectfiles, [('subject_id', 'subject_id')]),
                 (selectfiles, bet, [('mag', 'in_file')]),
                 (selectfiles, phs_range, [('phs', 'in_file')]),
                 (bet, datasink, [('mask_file', 'preproc.@bet')]),
                 (phs_range, datasink, [('out_file', 'preproc.@phs_range')]),
                 ])
