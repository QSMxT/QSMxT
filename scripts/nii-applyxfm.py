...
if __name__ == "__main__":
    ...
    sys_cmd("mkdir .tmp")
    if ".nii.gz" in args.in_file: sys_cmd(f"gunzip -f -k -c {args.in_file} > .tmp/{os.path.basename(args.in_file).split(os.extsep)[0]}.nii")
    else: sys_cmd(f"cp {args.in_file} .tmp/{os.path.basename(args.in_file).split(os.extsep)[0]}.nii")
    if ".nii.gz" in args.in_like: sys_cmd(f"gunzip -f -k -c {args.in_like} > .tmp/{os.path.basename(args.in_like).split(os.extsep)[0]}.nii")
