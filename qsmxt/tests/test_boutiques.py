...
CONFIG_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'docs', '_config.yml')


def _get_container_image():
    """Read the container image name from docs/_config.yml (single source of truth)."""
    if not os.path.exists(CONFIG_PATH):
        return None
    with open(CONFIG_PATH, 'r') as f:
        content = f.read()
    version_match = re.search(r'^TEST_CONTAINER_VERSION:\s*(.+)$', content, re.MULTILINE)
    date_match = re.search(r'^TEST_CONTAINER_DATE:\s*(.+)$', content, re.MULTILINE)
    if version_match and date_match:
        return f"vnmd/qsmxt_{version_match.group(1).strip()}:{date_match.group(1).strip()}"
    return None


def _get_patched_descriptor_path():
    """Return path to a descriptor with the container image set from _config.yml.

    The committed descriptor has image: null. This creates a temporary copy
    with the image filled in, avoiding race conditions with concurrent git
    operations on the original file.
    """
    ...
    patched_path = os.path.join(gettempdir(), "qsmxt_patched_descriptor.json")
...
class TestBoutiquesDescriptor:
    ...
    def test_boutiques_validate(self):
        ...
        patched = _get_patched_descriptor_path()
        # This will raise an exception if validation fails
        bosh(['validate', patched])
    ...
    def test_boutiques_simulate(self):
        ...
        result = bosh(['exec', 'simulate', '-i', TEST_INVOCATION_PATH, patched])
...
class TestBoutiquesIntegration:
    ...
    def test_boutiques_exec_launch(self):
        ...
        try:
            ...
            bosh(['exec', 'launch', '--no-container', patched, invocation_path])
    ...
    def test_boutiques_invocation_validation(self):
        ...
        try:
            ...
            result = bosh(['exec', 'simulate', '-i', invocation_path, patched])
            assert result is not None
