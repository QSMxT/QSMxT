...
class TestBoutiquesIntegration:
    ...
    def bids_dir(self):
        """
        Generate a synthetic BIDS directory for testing.

        Uses qsm_forward to create realistic test data from head phantom maps.
        """
        import numpy as np
        import qsm_forward
        from qsmxt.tests.qsmxt.integration.utils import download_from_osf
        from qsmxt.scripts.sys_cmd import sys_cmd

        tmp_dir = gettempdir()
        bids_dir = os.path.join(tmp_dir, "bids-boutiques-test")

        # Check if we already have test data
        if not glob.glob(os.path.join(bids_dir, "sub*")):
    ...
    def test_boutiques_exec_launch(self, bids_dir):
        """
        Test actual execution of QSMxT via bosh exec launch.

        This test:
        1. Generates a synthetic BIDS dataset
        ...
        invocation_path = os.path.join(gettempdir(), "boutiques_integration_invocation.json")
        ...
        try:
            ...
            assert len(chimap_files) > 0, "No Chimap output files found"
